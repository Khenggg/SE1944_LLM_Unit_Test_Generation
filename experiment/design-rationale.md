...
|---|---|---|
| LLM/Tool | GPT-4 (with potential reference tests on GPT-3.5/GPT-4o) | GAP-T: Tool/LLM column (GPT-4 is the most widely used SOTA model, yet lacks large-scale direct comparison against actual developer tests) |
...
| Baseline type | Absolute threshold & Human-level baseline | RQ Objective: Must achieve specific percentages (≥ 80% branch, ≥ 60% mutation) and directly compare against human-written tests |
| Threshold RQ1 | Branch coverage ≥ 80% | Case 2: Based on paper SS040 where GPT-4 achieves 74% coverage on real functions, we round the expectation up to 80% to ensure production standard |
| Threshold RQ2 | Mutation score ≥ 60% | Case 2: Based on paper SS040 reporting a 58% mutation score for GPT-4, we set the target milestone at 60% |
...
Threshold Branch coverage 80% — Case 2 — floor = 74% from paper SS040 (2026). Rationale: SS040 evaluates GPT-4 on the ULT benchmark containing complex real-world functions, obtaining a 74% coverage result. To ensure that automatically generated unit tests approach the quality of actual production code, the research sets a challenging milestone that GPT-4 can reach the rounded-up threshold of 80% for functions with medium complexity.

Threshold Mutation score 60% — Case 2 — floor = 58% from paper SS040 (2026). Rationale: Similarly, SS040 records a mutation score of 58% with GPT-4. The 60% threshold is selected as a milestone to determine if the automated generation possesses sufficient semantic fault-detection capability closely comparable to developer-written tests.

## Pipeline Rationale

The evaluation pipeline must ensure fairness and reproducibility when comparing AI-generated code against human-written code. The LLM models will be provided with full context including the function signature and source code descriptions, after which the test generator will automatically design test cases. The output will undergo an automated system for compiling, running coverage, and injecting mutations for measurement.
