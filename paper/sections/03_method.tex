...
For every SUT, JaCoCo produced branch-covered and branch-total counts. Branch coverage is the covered-to-total ratio. PIT produced killed-mutant and eligible-mutant counts; mutation score is their ratio. A suite was executable only if its test class compiled and passed. The report retains zero or unavailable class-level outcomes for non-executable suites as defined by the pipeline, while the paired RQ3 analysis includes only SUTs that passed in both tools.
