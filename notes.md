...
5.  Xem kết quả bằng cách quay lại thư mục root và chạy:
    ...
    python scripts/parse_metrics.py
    ```
6.  **Sao lưu và dọn dẹp bộ test EvoSuite (Sau khi đo đạc xong):**
    *   **Sao lưu bộ test EvoSuite 1 phút:**
        ```powershell
        python scripts/manage_tests.py --action archive --tool evosuite --type pilot --interval 1m
        ```
        *(Các test case sẽ được di chuyển tự động từ thư mục chính vào thư mục lưu trữ ngoài `experiments/rbl-project/evosuite_backups/pilot/1m/run_X_[timestamp]/`)*
