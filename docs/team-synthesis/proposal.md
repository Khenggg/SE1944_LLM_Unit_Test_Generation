...
Trong quy trình kỹ nghệ phần mềm và giảng dạy kiểm thử đơn vị (unit testing) cho sinh viên, việc tạo ra các bộ kiểm thử đơn vị có độ bao phủ cấu trúc và năng lực phát hiện lỗi cao là vô cùng quan trọng nhằm đảm bảo chất lượng mã nguồn. Tuy nhiên, việc viết kiểm thử thủ công thường tiêu tốn nhiều thời gian và dễ bỏ sót các trường hợp biên quan trọng [GS017, GS026]. Việc ứng dụng các mô hình ngôn ngữ lớn (LLM) để tự động hóa sinh test suite hứa hẹn sẽ tối ưu hóa quy trình kiểm thử phần mềm [GS010].

### 2.2 State of the Art
Các nghiên cứu của Huang'26 [GS010] và AgoneTest'25 [GS024] đã chứng minh năng lực đáng kể của LLM thế hệ mới (như GPT-4, LLaMA-3) trong việc sinh mã kiểm thử. Nhiều công cụ sinh test dựa trên LLM (ví dụ như TestPilot của Al-Tse'23 [GS005]) đã đạt được độ bao phủ dòng và bao phủ nhánh cao trên các tập dữ liệu chuẩn hoặc mã nguồn mở [GS004, GS011]. Dù vậy, hiệu năng thực tế của mã kiểm thử sinh bởi AI khi đối chứng trực tiếp với kiểm thử viết tay bởi con người ở cấp độ đơn vị lập trình độc lập (standalone functions) vẫn còn nhiều tranh cãi [GS019, GS024]. Đặc biệt, các nghiên cứu lớn như MutGen'26 [GS026] và Al-Tse'23 [GS005] cảnh báo rằng độ bao phủ cao (coverage) không phải lúc nào cũng đồng hành với năng lực bắt lỗi logic (mutation score).

### 2.3 GAP
Dựa trên phân tích bảng tổng hợp văn liệu của nhóm ($N = 39$ papers), hầu hết các nghiên cứu tập trung đánh giá hiệu năng sinh kiểm thử của LLM trên các tập dữ liệu công nghiệp hoặc so sánh LLM với các công cụ sinh test tự động truyền thống (như EvoSuite hoặc Randoop) [GS011, GS020]. Chi tiết phân tích GAP phản chứng được nhóm thực hiện tại [gap-statement-final.md](file:///f:/Ky%205/SWT301/Github/SE1944_LLM_Unit_Test_Generation/team-synthesis/gap-statement-final.md) và bảng văn liệu gộp tại [evidence-table-merged.md](file:///f:/Ky%205/SWT301/Github/SE1944_LLM_Unit_Test_Generation/team-synthesis/evidence-table-merged.md).
*   **GAP-D (Primary):** Hiện chưa có nghiên cứu nào thực hiện đánh giá và so sánh cặp (paired comparison) giữa bộ kiểm thử sinh bởi GPT-4o-mini với bộ kiểm thử đối chứng viết tay của sinh viên (student-written benchmark tests) trên cùng một tập các hàm Java của benchmark HumanEval-Java.
*   **GAP-M (Secondary):** Việc đánh giá chất lượng test suite sinh bởi LLM thường bị thiên lệch nếu chỉ sử dụng độ bao phủ nhánh mà thiếu đi sự đánh giá độc lập về kiểm thử đột biến (mutation score) để xác minh năng lực phát hiện lỗi logic thực tế [GS026].
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **GS010** (Huang'26) | 12 LLMs (GPT-4, LLaMA) | ULT (3,909 Python functions) | Branch coverage, Mutation score | GPT-4 đạt 74% coverage, 58% mutation score | Chi phí API rất cao; chưa đối chứng với con người |
| **GS005** (Al-Tse'23) | TestPilot (GPT-3.5) | 25 npm packages (1,684 JS functions) | Statement & Branch coverage | 70.2% Statement, 52.8% Branch coverage | Chỉ đánh giá JavaScript; không đo điểm đột biến |
| **GS024** (AgoneTest'25) | GPT-4o-mini, Gemini, LLaMA3 | Classes2Test (147k Java classes) | Branch coverage, Mutation score | LLaMA3.1 70B đạt 79.8% BC, 89.2% MS | Dữ liệu lớp lớn, tỷ lệ biên dịch ban đầu thấp |
| **GS026** (MutGen'26) | MUTGEN (LLM + Mutation feedback) | 204 Java subjects | Code coverage, Mutation score | Đạt mutation score vượt trội EvoSuite | Vòng lặp phản hồi đột biến tốn nhiều tài nguyên |
| **GS019** (Multi-Agent'26) | GPT-4, GPT-3.5 multi-agent | HumanEvalJava, LeetCodeJava | Branch coverage, Mutation score | Năng lực tương đương với testcase con người | Cần nhiều agent đồng thuận gây trễ và tốn token |

### 3.2 Pattern Analysis
Qua phân tích văn liệu, nhóm rút ra 3 nhận xét tổng quan sau:
1.  **Sự thiên lệch về độ bao phủ cấu trúc:** Hầu hết các nghiên cứu tập trung đo đạc độ bao phủ dòng/nhánh làm thước đo chất lượng test suite [GS005, GS011]. Tuy nhiên, các paper mới như **GS026** chỉ ra hiện tượng "blindness" khi test suite đạt 100% coverage nhưng chỉ đạt 4% mutation score vì các câu lệnh assertion quá yếu hoặc rỗng.
2.  **Sự thiếu hụt đối chứng với con người:** Đa số nghiên cứu đối chứng LLM với EvoSuite hoặc Randoop [GS011, GS020]. Rất ít công trình thực hiện so sánh trực tiếp với testcase do con người/sinh viên tự viết trên cùng một tập đơn vị kiểm thử cụ thể [GS019, GS024].
3.  **Tác động của độ phức tạp của hàm (CC):** Chất lượng kiểm thử sinh bởi AI giảm mạnh khi Cyclomatic Complexity của hàm tăng lên, đặc biệt là khi phát sinh các dependency phức tạp giữa các lớp [GS017, GS036].

### 3.3 GAP Mapping

| GAP Type | Evidence (Số paper hỗ trợ) | Trạng thái của GAP |
| :--- | :--- | :--- |
| **GAP-T** (Frontier model evaluation) | 5 papers (GS004, GS017, GS023, GS024, GS036) | Confirmed-Deferred (Đã có nghiên cứu dùng GPT-4o-mini) |
| **GAP-D** (Paired Student comparison on Java) | 0 papers | **Confirmed (Primary GAP của nhóm)** |
| **GAP-M** (Dual adequacy evaluation: BC + MS) | 12 papers (GS010, GS017, GS019, GS024, GS026) | **Confirmed (Secondary GAP của nhóm)** |
...
*   **Ngưỡng:** $\ge 30.22\%$ - **Case 2** (Floor value thực nghiệm trung bình của các mô hình LLM trên dữ liệu thực tế ULT từ paper **GS010**).
...
*   **Ngưỡng:** $4.0\%$ - **Case 2** (Ngưỡng sàn lỗi logic tối thiểu từ paper **GS026**); và $40.21\%$ - **Case 2** (Hiệu năng trung bình trên ULT từ paper **GS010**).
*   **Statistical test:** One-sample Wilcoxon signed-rank test ($\alpha = 0.05$, một đuôi).
...
2.  **LLM Prompting:** Sử dụng API OpenAI gửi prompt sinh unit test bằng `gpt-4o-mini-2024-07-18` với `temperature = 0`. Testcase sinh ra được chỉ định gọi chính xác các lớp trong package `humaneval.correct` để đo đạc chất lượng mã chuẩn.
...
*   **Model:** `gpt-4o-mini-2024-07-18` (OpenAI API chính thức).
*   **Hyperparameters:** `temperature = 0` (đảm bảo tính deterministic và khả năng tái lập kết quả), `top_p = 1`, `max_tokens = 2000`.
...
### 5.4 Measurement
*   **Metric chính 1:** Branch Coverage - Đo bằng công cụ JaCoCo (phiên bản `0.8.7+`), tích hợp qua `jacoco-maven-plugin`.
*   **Metric chính 2:** Mutation Score - Đo bằng PIT/PiTest (phiên bản `1.7.4+`), sử dụng cấu hình tập toán tử đột biến cơ bản (mutators: `DEFAULTS`) để tránh quá tải tài nguyên phần cứng.
*   **Metric phụ:** Compile & Execution Success Rate (tỷ lệ biên dịch và chạy qua suôn sẻ).
*   **Xác định Ground Truth và IAA:** Vì tập dữ liệu HumanEval-Java cung cấp sẵn mã nguồn chuẩn (Correct Java Code) làm Ground Truth, thực nghiệm này thực hiện đo đạc tự động thông qua công cụ JaCoCo và PiTest. Quá trình đo lường không sử dụng chuyên gia gắn nhãn thủ công (Human Annotation), do đó chỉ số đồng thuận giữa các người đánh giá (IAA) không áp dụng.
...
*   **Phép kiểm định thống kê:**
    *   **Wilcoxon Signed-Rank Test (One-sample):** Dùng cho RQ1 và RQ2 để so sánh trung vị mẫu ($N = 50$) với các giá trị hằng số $30.22\%$, $4.0\%$ và $40.21\%$. Đây là phép kiểm định phi tham số an toàn vì dữ liệu tỷ lệ độ bao phủ/đột biến thường bị giới hạn trong đoạn $[0, 1]$ và phân phối lệch chuẩn.
...
*   **Mối đe dọa (Generalizability / Dataset bias):** Tập dữ liệu gồm 50 hàm Java từ HumanEval-Java chủ yếu là các bài toán thuật toán cô lập và có thể không đại diện cho các lớp Java phức tạp trong hệ thống phần mềm doanh nghiệp thực tế.
